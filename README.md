...
- While VS Code can handle Python code just fine out of the box, to be most effective when writing code you'll want to add an extension with [additional Python support](https://code.visualstudio.com/docs/languages/python). We recommend [Don Jayamanne's](https://marketplace.visualstudio.com/items?itemName=donjayamanne.python) extension: you can easily install this by using the command-palette (open it up and type `"extension"` to find the "Install Extensions" option),
